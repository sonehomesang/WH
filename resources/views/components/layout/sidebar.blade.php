@php
    ...
    // Order & grouping per user's sidebar layout (2026-09-14). 'group' draws a divider on change.
    $nav = [
        // ── ກຸ່ມ 1 · ສາງ & ທຸລະກຳ ຫຼັກ ──
        ['group' => 1, 'menu' => 'dashboard', 'label' => 'Dashboard', 'route' => route('dashboard'), 'always' => true,
        ...
        ['group' => 1, 'menu' => 'inventory', 'label' => 'WH Inventories', 'route' => route('inventory'),
            'icon' => 'M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9'],
        ['group' => 1, 'menu' => 'borrow', 'label' => 'Borrowing Material/Tools', 'route' => route('borrow'),
            'icon' => 'M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5'],
        ['group' => 1, 'menu' => 'deposit', 'label' => 'Deposit Material/Equipment', 'route' => route('deposit'),
        ...
        ['group' => 1, 'menu' => 'disposal', 'label' => 'Disposal · ຈຳໜ່າຍ', 'route' => route('disposal'),
            'icon' => 'M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0'],
        ['group' => 1, 'menu' => 'catalog', 'label' => 'Shops Material', 'route' => route('catalog'),
        ...
        ['group' => 1, 'menu' => 'request', 'label' => 'Request Material', 'route' => route('request'),
            'icon' => 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25Z'],
        ['group' => 1, 'menu' => 'ansi', 'label' => 'New Stock Item (ANSI)', 'route' => route('ansi'),
            'icon' => 'M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z'],
        // ── ກຸ່ມ 2 · ແຈ້ງ supplier ──
        ['group' => 2, 'menu' => 'da', 'label' => 'DA Claims', 'route' => route('da'),
            'icon' => 'M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z'],
        ['group' => 2, 'menu' => 'oga', 'label' => 'OGA', 'route' => route('oga'),
            'icon' => 'M6 12 3.269 3.125A59.769 59.769 0 0121.485 12 59.768 59.768 0 013.27 20.875L5.999 12Zm0 0h7.5'],
        // ── ກຸ່ມ 3 · ຊັບສິນ · ກວດກາ · ກິດຈະກຳ ──
        ['group' => 3, 'menu' => 'equipment', 'label' => 'Equipment & Tools', 'route' => route('equipment'),
            'icon' => 'M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 1 1-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 0 0 4.486-6.336l-3.276 3.277a3.004 3.004 0 0 1-2.25-2.25l3.276-3.276a4.5 4.5 0 0 0-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437 1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008Z'],
        ['group' => 3, 'menu' => 'area_inspection', 'match' => 'area-inspection', 'label' => 'ກວດ ສະຖານທີ່', 'route' => route('area-inspection'),
            'icon' => 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25Z'],
        ['group' => 3, 'menu' => 'expo', 'label' => 'Expo Info', 'route' => route('expo'),
        ...
        <nav class="px-2 py-3 space-y-0.5">
            ...
            @foreach ($nav as $item)
                @if ((($item['always'] ?? false) || $u->can($item['menu'].'.view')) && \App\Support\Modules::enabled($item['menu']))
                    @php $g = $item['group'] ?? 1; @endphp
                    @if ($lastGroup !== null && $g !== $lastGroup)
                        <div class="my-2 border-t border-gray-800" aria-hidden="true"></div>
                    @endif
                    @php $base = $item['match'] ?? $item['menu']; $active = $item['route'] !== '#' && (request()->routeIs($base) || request()->routeIs($base.'.*')); @endphp
            ...
            @canany(['survey.view', 'reports.view'])
                <a href="{{ route('survey.results') }}" wire:navigate
                   class="{{ $linkBase }} {{ request()->routeIs('survey.results') ? $linkActive : $linkIdle }}">
